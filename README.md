...
| | |
|---|---|
| 🛰️ **主管调度** | LangGraph decide↔tools 决策循环：拆解任务 → 逐个调成员 → 汇总；对话流实时展示各 agent 活动 |
...
```
用户提问 → Supervisor（LangGraph：decide ↔ tools 决策循环，超轮次强制收尾）
   ...
   → SSE 事件流：activity（活动日志）→ delta（流式回答）→ sources（聚合来源）→ done
```
